...
class POLYGROUPS_PG_seam_preparation_settings(bpy.types.PropertyGroup):
    seam_path_pin: bpy.props.BoolProperty(
        name="Mark As Pinned", description="Pin seams created by Vertex Seam Path and Edge Seam Path", default=False)
    seam_eraser_clear_mode: bpy.props.EnumProperty(
        name="Clear", description="Choose whether erasers clear ordinary seams or pinned seams only",
        items=(("SEAMS", "Clear Seams", "Clear ordinary seams while preserving pinned seams"),
               ("PINNED", "Clear Pinned Only", "Clear both seam and pin marks from pinned seams only")), default="SEAMS")
    ...
    smart_seam_pin_generated: bpy.props.BoolProperty(
        name="Mark Generated Seams As Pinned", description="Pin every seam generated by Smart Mark Seams", default=False)
